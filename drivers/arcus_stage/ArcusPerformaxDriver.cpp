       ...
       if (libusb_open(list[i], &device_handle) != 0 ) {
           ...
           if (dwOptions == PERFORMAX_RETURN_SERIAL_NUMBER) {
               if (0 > libusb_get_string_descriptor_ascii(
                       device_handle, descriptor.iSerialNumber, reinterpret_cast<unsigned char*>(lpDeviceString), PERFORMAX_MAX_DEVICE_STRLEN)) {
                   result = AR_FALSE; // invalid descriptor
               }
           } else if (dwOptions == PERFORMAX_RETURN_DESCRIPTION) {
               if (0 > libusb_get_string_descriptor_ascii(
                       device_handle, descriptor.iProduct, reinterpret_cast<unsigned char*>(lpDeviceString), PERFORMAX_MAX_DEVICE_STRLEN)) {
                   result = AR_FALSE; // invalid descriptor
               }
           } else {
...
char buffer[4096];
    ...
    result = libusb_bulk_transfer(device_handle, 0x82, reinterpret_cast<unsigned char*>(buffer), sizeof(buffer), &transferred, libusb_ReadTimeout);
    // if the above fails, it's probably ok.  We probably don't care.

    result = libusb_bulk_transfer(device_handle, 0x02, reinterpret_cast<unsigned char*>(wBuffer), dwNumBytesToWrite, &transferred, libusb_WriteTimeout);
    ...
    result = libusb_bulk_transfer(device_handle, 0x82, reinterpret_cast<unsigned char*>(rBuffer), dwNumBytesToRead, &transferred, libusb_ReadTimeout);
